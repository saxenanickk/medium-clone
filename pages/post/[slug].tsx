...
function Post({ post }: Props) {
  ...
  const postImage: any = useNextSanityImage(
    sanityConfig as any,
    post.mainImage
  );
  const authorImage: any = useNextSanityImage(
    sanityConfig as any,
    post.author.image
  );
  ...
  return (
    <main>
      <Header />
      <img className="w-full h-40 object-cover" src={postImage?.src!} alt="" />
        ...
        <div>
          <PortableText
            ...
            serializers={{
              ...
              image: ({ asset }: any) => {
                const image: any = useNextSanityImage(
                  sanityConfig as any,
                  asset
                );
...
export const getStaticProps: GetStaticProps = async ({ params }) => {
  ...
  return {
    ...
    revalidate: 60, // It will update the cached version every 60 seconds
  };
};
